...
namespace TokoBuku.BaseForm.EditForm
{
    public static class FormEdit
    {
        ...
        public static FormInputDataBarang Barang(DataGridViewRow row)
        {
            ...
            form.NamaBarang = row.Cells[2].Value.ToString();
            form.KodeBarang = row.Cells[1].Value.ToString();
            form.Penerbit = row.Cells[3].Value.ToString();
            form.Kategori = row.Cells[4].Value.ToString();
            form.Rak = row.Cells[5].Value.ToString();
            form.Stock = Convert.ToInt32(row.Cells[6].Value.ToString());
            form.Harga = Convert.ToDouble(row.Cells[7].Value.ToString());
            form.ISBN = row.Cells[8].Value.ToString();
            form.Penulis = row.Cells[9].Value.ToString();
            form.Diskon = Convert.ToDouble(row.Cells[10].Value.ToString());
            form.Status = row.Cells[11].Value.ToString();
            form.BarCode = row.Cells[12].Value.ToString();
            form.SetToEditForm();
            return form;
        }
